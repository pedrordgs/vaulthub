...
import { encrypt } from "@/app/actions/vault";
...
export function EncryptForm() {
  ...
  const handleSubmit = useCallback((formData: FormData) => {
    startTransition(async () => {
      const response = await encrypt(formData);
      if (response.success) {
    ...
    <div className="space-y-6">
      <form action={handleSubmit} className="space-y-5" onReset={handleReset}>
        ...
        <div className="space-y-2">
          <Label htmlFor="plainText" className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
            Plain text
          </Label>
          <Textarea
            id="plainText"
